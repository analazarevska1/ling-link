...
@section('content')
    <div class="relative w-full h-32 overflow-hidden px-4 md:px-20">
        <img src="{{ asset('images/contact-img.jpg') }}" alt="{{ __('contact.hero_title') }}"
            class="w-full h-full brightness-70 rounded-3xl object-cover" style="object-position: center 25%;">
    ...
    <div class="w-full px-4 md:px-20 py-10">
        <div class="flex flex-col md:flex-row gap-6">
            ...
            <div class="flex-1 bg-white rounded-3xl p-8 shadow-2xl">
                <h2 class="text-2xl font-extrabold uppercase tracking-wide mb-2">{{ __('contact.form_title') }}</h2>
                <p class="text-gray-500 text-sm mb-6">{{ __('contact.form_desc') }}</p>

                <form method="POST" action="{{ route('prijava.store', 'контакт') }}">
                    @if (session('success'))
                        <div class="bg-green-100 text-green-700 rounded-xl px-4 py-3 mb-4 text-sm"
                    ...
                    <div class="flex flex-col md:flex-row gap-4 mb-4">
                        <div class="flex-1">
                            <label class="block font-bold text-sm mb-1">{{ __('contact.name') }}</label>
                        ...
                        <div class="flex-1">
                            <label class="block font-bold text-sm mb-1">{{ __('contact.phone') }}</label>
                            <input type="text" name="phone" placeholder="{{ __('contact.phone_placeholder') }}"
                                class="w-full border border-gray-300 rounded-full px-4 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-900">
                        </div>
                    </div>
                    ...
                    {{-- Email --}}
                    <div class="mb-4">
                        <label class="block font-bold text-sm mb-1">{{ __('contact.email') }}</label>
                    ...
                    {{-- Message --}}
                    <div class="mb-6">
                        <label class="block font-bold text-sm mb-1">{{ __('contact.message') }}</label>
                        <textarea rows="7" name="message" placeholder="{{ __('contact.message_placeholder') }}"
                    ...
                    <div class="flex justify-center">
                        <button type="submit"
                            class="text-white px-10 py-4 rounded-full font-semibold hover:bg-blue-800 transition"
                            style="background-color: #194077;">
                            {{ __('contact.submit') }}
                        </button>
                    </div>
                </form>
            ...
            <div class="flex-1 rounded-3xl p-8 text-white" style="background-color: #194077;">
                <h2 class="text-2xl font-extrabold uppercase tracking-wide mb-2">{{ __('contact.info_title') }}</h2>
                <p class="text-blue-200 text-sm mb-8">{{ __('contact.info_desc') }}</p>
                ...
                <div class="flex items-start gap-4 mb-6">
                    ...
                    <div>
                        <p class="font-bold">{{ __('contact.location') }}</p>
                ...
                <div class="flex items-start gap-4 mb-6">
                    ...
                    <div>
                        ...
                        <a href="http://www.lingualink.com.mk/" class="text-blue-300 text-sm hover:underline">
                            http://www.lingualink.com.mk/
                        </a>
                    </div>
                </div>
