@php $cards = __('benefits.cards'); @endphp

<style>
    .benefit-wrapper { position: relative; height: 260px; }
    .benefit-card { position: absolute; inset: 0; border-radius: 18px; display: flex; flex-direction: column; justify-content: center; padding: 32px; transition: bottom 0.35s ease; bottom: 0; }
    .benefit-btn { position: absolute; bottom: 0; left: 0; right: 0; display: flex; justify-content: center; opacity: 0; transition: opacity 0.35s ease; }
    .benefit-wrapper:hover .benefit-card { bottom: 60px; }
    .benefit-wrapper:hover .benefit-btn  { opacity: 1; }
    @media (max-width: 768px) {
        .benefit-wrapper { height: auto; position: static; }
        .benefit-card { position: static; border-radius: 18px; padding: 24px 20px; height: auto; bottom: auto; transition: none; }
...
@php
$bgColors = ['#84CDF1', '#E5F7FF', '#84CDF1', '#E5F7FF', '#84CDF1'];
@endphp
...
<section class="hidden md:block" style="background-color: #194077; padding: 64px 150px; margin: 40px 0; width: 100%;">
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 24px; margin-bottom: 24px;">
        ...
        <div style="display: flex; flex-direction: column; justify-content: center; padding-right: 40px; height: 260px;">
            ...
            <p style="font-size: 0.88rem; color: #cce4f0; line-height: 1.6;">
                {{ __('benefits.subtitle') }}
            </p>
        </div>

        @foreach(array_slice($cards, 0, 2) as $i => $card)
        <div class="benefit-wrapper">
            <div class="benefit-card" style="background-color: {{ $bgColors[$i] }};">
            ...
            <div class="benefit-btn">
                <button onclick="openBenefitModal({{ $i }})" style="width:90%;background:#fff;border:none;cursor:pointer;font-size:0.9rem;font-weight:500;color:#1a1a1a;padding:14px;border-radius:50px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                    {{ __('benefits.read_more') }}
                </button>
            </div>
        </div>
        @endforeach
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 24px;">
        ...
        <div class="benefit-wrapper">
            <div class="benefit-card" style="background-color: {{ $bgColors[$i + 2] }};">
                <p style="font-size: 1rem; font-weight: 700; color: #1a1a1a; margin-bottom: 10px; line-height: 1.4;">{{ $card['title'] }}</p>
                <p style="font-size: 0.88rem; color: #1a1a1a; line-height: 1.5;">{{ $card['desc'] }}</p>
            </div>
            <div class="benefit-btn">
                <button onclick="openBenefitModal({{ $i + 2 }})" style="width:90%;background:#fff;border:none;cursor:pointer;font-size:0.9rem;font-weight:500;color:#1a1a1a;padding:14px;border-radius:50px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
                    {{ __('benefits.read_more') }}
                </button>
            </div>
        </div>
...
<div id="benefitModal" onclick="closeBenefitModal()"
    ...
    <div onclick="event.stopPropagation()"
         style="background:#fff; border-radius:16px; padding:36px 28px; max-width:580px; width:90%; position:relative; text-align:center;">
        <button onclick="closeBenefitModal()"
                style="position:absolute; top:16px; right:20px; background:none; border:none; font-size:1.4rem; cursor:pointer; color:#333;">✕</button>
...
<script>
    ...
    document.addEventListener('keydown', (e) => { if (e.key === 'Escape') closeBenefitModal(); });
</script>